...
def SAP_UDP_BIDI():
	...
	for dev in adb_dev:
		if dev != SAP_ID :
			...
			UL_server_cmd=" gnome-terminal -e 'adb -s "+SAP_ID+" shell iperf -s -u -i 1' "
			UL_client_cmd=" gnome-terminal -e 'adb -s "+STA_ID+" shell iperf -c "+SAP_IP+" -u -b "+Band_width+" -i 1 -t "+Time_limit+" '"
			DL_server_cmd=" gnome-terminal -e 'adb -s "+STA_ID+" shell iperf -s -u -i 1' "
			DL_client_cmd=" gnome-terminal -e 'adb -s "+SAP_ID+" shell iperf -c "+STA_IP+" -u -b "+Band_width+" -i 1 -t "+Time_limit+" '"

			os.system(UL_server_cmd)
			os.system(UL_client_cmd)
			os.system(DL_server_cmd)
			os.system(DL_client_cmd)
			time.sleep(2)
		else:
			pass
